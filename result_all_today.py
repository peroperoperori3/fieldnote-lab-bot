...
# ====== keiba.go.jp 結果（RaceMarkTable：上位3のみ） ======
...
# ====== HTML（予想の相対色分けロジックに完全追従 + 混戦度バッジ + 収支サマリ） ======
...
def main():
    ...
    print(f"[INFO] BET bet_unit={BET_UNIT} box_n={BET_BOX_N}")
    ...
    for track in active:
        ...
        if debug:
            print(f"[DEBUG] refundmoney={ref_url}  races_with_sanrenpuku={len(sanrenpuku_map)}")
